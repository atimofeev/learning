...
def print_header(row, col_widths):
    ...
    for idx, col in enumerate(row):
        if idx < 7:
            print(ljustcolor(HEADER_COLOR + col.strip() +
                             RESET_COLOR, col_widths[idx]), end=" ")
...
def print_row(row, col_widths):
    """Print non-header row."""
    output_row = []
    for idx, col in enumerate(row):
        if idx == 0:
            output_row.append(ljustcolor(USERID_COLOR + col +
                                         RESET_COLOR, col_widths[idx]))
        elif idx < 7:
...
def process_cmd(col):
    ...
    if ascii_tree_match:
        ...
        col = col.replace(ascii_tree, TREE_ASCII_COLOR + ascii_tree +
                          RESET_COLOR, 1)
    system_proc_match = re.search(r'(\[[\w/:.+-.]+\])', remaining_part)
    if system_proc_match:
        system_proc = system_proc_match.group(1)
        col = col.replace(system_proc, SYSTEM_PROC_COLOR + system_proc +
                          RESET_COLOR, 1)
    else:
        cmd_parts = remaining_part.split(' ', 1)
        col = col.replace(cmd_parts[0], COMMAND_COLOR + cmd_parts[0] +
                          RESET_COLOR, 1)
        if len(cmd_parts) > 1:
            col = col.replace(cmd_parts[1], ARGUMENT_COLOR + cmd_parts[1] +
                              RESET_COLOR, 1)
    return col
